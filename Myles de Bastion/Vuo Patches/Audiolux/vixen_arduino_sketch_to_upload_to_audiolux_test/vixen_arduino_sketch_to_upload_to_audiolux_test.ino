...
void loop() {
  ...
  for(;;) {
    // Set a counter to 0.  This counter keeps track of the pixel colors received.
    cnt = 0;
    ...
    do {
      while(!Serial.available());
        ...
        Serial.print("R= ");
        Serial.println(leds[cnt].r);
      while(!Serial.available());
        leds[cnt].g = Serial.read();
        Serial.print("G= ");
        Serial.println(leds[cnt].g);
      while(!Serial.available());
        leds[cnt++].b = Serial.read();
        Serial.print("B= ");
        Serial.println(leds[cnt].b);
      }
    while(--num_leds);
