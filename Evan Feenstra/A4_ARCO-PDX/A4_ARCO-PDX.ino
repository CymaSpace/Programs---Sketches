...
#define NUM_LEDS 61 // Number of LED's in the strip
...
int refresh_rate = 4; // Refresh rate of the animation
...
void loop() {
  ...
  if(stomp_pressed()) {
    ...
    if(refresh_rate>12){refresh_rate=4;} //either 4 or 10
  ...
  start_hue = analogRead(REFRESH_POT_PIN) / 4 - 30;
  //Serial.println(refresh_rate); 3/11
...
void set_LED_color(int position, int value) {
  ...
  color.hue = start_hue + ((ratio * 255) * 1.2);
  leds[position] = color;
}
