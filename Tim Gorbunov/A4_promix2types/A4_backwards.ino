...
void runleds_A4BW(){
  ...
  if(refresh_counter >= refresh_rate) {
     ...
     */
    push_stack_A4BW(left_LED_stack_A4BW, amp_sum_L);
    push_stack_A4BW(right_LED_stack_A4BW, amp_sum_R);
    ...
    for(i = LEFT_START_POINT; i >= LEFT_END_POINT; --i) {
      set_LED_color_A4BW(i, left_LED_stack_A4BW[stack_loop]);
      ++stack_loop;
    }
    ...
    for(i = RIGHT_START_POINT; i <= RIGHT_END_POINT; ++i) {
      set_LED_color_A4BW(i, right_LED_stack_A4BW[stack_loop]);
      ++stack_loop;
    }

    leds_A4BW[0] = left_LED_stack_A4BW[0];
