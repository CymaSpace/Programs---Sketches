...
{  
    if (digitalRead(stomp) == HIGH || firstt){
      firstt = false;
      delay(200);
      for (int o=1; o<=CNT_LIGHTS; o++)
        strip.setPixelColor(o,0,0,0);
...
void runTime()
{
  ...
  int use_le = 0;
  int use_ls = ((CNT_LIGHTS/2)-1+cenoffset);
  int use_rs = ((CNT_LIGHTS/2)+0+cenoffset);
...
//Serial.println(spectrum_totalR);
  ...
  if(refresh_counter>=(use_refresh - round(tmp_refresh_adj * .1)))
  {
    ...
    num = prop[CNT_LIGHTS/2];
    if(num>=0)
      {
        get_color();
        strip.setPixelColor( 0, useColor[0], useColor[1], useColor[2]);
      }//if
  }//if refresh  
