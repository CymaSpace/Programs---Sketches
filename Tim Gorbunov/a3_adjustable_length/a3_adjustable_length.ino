...
int analogPinpot = 2;// dial for length
...
{  
   ...
   runTime();
   useLength = map(analogRead(analogPinpot), 0, 1023, CNT_LIGHTS, 2);
   //Serial.println(useLength);
...
void runTime()
{
  ...
  int use_le = 0;
  int use_ls = ((useLength/2)-1);
  int use_rs = ((useLength/2)+0);
  int use_re = (useLength-1);
