...
#include <avr/power.h>
#define PIN 6
#define actCNT_LIGHTS 120 // the actual number of lights represented
#define CNT_LIGHTS 120 //number of led desired
...
int skipnum = actCNT_LIGHTS / CNT_LIGHTS;
...
int leddat[CNT_LIGHTS][3]; //colors at each led
...
//leddat[20][1] = 255;
  ...
  for (int h =0; h<7; h++){
    for (int i=leds[h][3]; i>leds[h][3]-ledw; i--){ //from current position to length of tail
     for (int k=0; k<3; k++){ //for each of the three colors
       if (i<0)//if going behind the strip
         if (leddat[CNT_LIGHTS+i][k] + leds[h][k] -((leds[h][k]/ledw)*(leds[h][3]-i+1)) >=255)//if the color is full
            leddat[CNT_LIGHTS+i][k] = 255;
         else
            leddat[CNT_LIGHTS+i][k] += leds[h][k] -((leds[h][k]/ledw)*(leds[h][3]-i+1));
       else
           if (leddat[i][k] + leds[h][k] -((leds[h][k]/ledw)*(leds[h][3]-i+1)) >=255) //if the color is full
             leddat[i][k] = 255;
           else
...
void setled(){
  for (int i=0; i<CNT_LIGHTS; i++){
    ...
    int b = leddat[i][2];
    if (i != 0)
      strip.setPixelColor(i*skipnum, r, g, b);
  }
  strip.show();
  clearleddat();
}
