...
#define STARTING_FREQ 800
#define ENDING_FREQ 14000
#define FFT_BANDS 512 // Don't change this unless the FFT library changes. This is the amount of audio data available for the Teensy Audio FFT
#define BAND_FREQ ((MAX_FREQ - MIN_FREQ) / FFT_BANDS)
#define MIN_FREQ 20
#define MAX_FREQ 22000 // Highest possible frequency from audio data
...
float get_FFT_vals(float bands[], float sum) {
  float amplitude_multiplier = 1.2; // This boosts the signal so that the summed FFT values are higher to increase reactivity
  int starting_band = STARTING_FREQ / BAND_FREQ; // Gives the first FFT band to start reading from
  int ending_band = ENDING_FREQ / BAND_FREQ;
  int total_range = ending_band - starting_band; // Total viable range of frequencies
  int step = total_range / BAND_FREQ; // How many bands to pull in for each column
  // Set sampling ranges based on function
  // 128 total bands of frequency
  for (int k=0; k<COLUMNS; k++){
    if(k >= int(COLUMNS * 0.6)) {
      float start = 0.05 * pow((k - 5), 2);
