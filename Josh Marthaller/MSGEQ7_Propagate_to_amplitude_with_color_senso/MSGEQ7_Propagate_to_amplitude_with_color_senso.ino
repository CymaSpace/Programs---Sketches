...
void get_color()
{
  ...
  int r,g,b;
  uint16_t t_r, t_g, t_b, c, colorTemp, lux;
  float largest = 0;
  float pct;
  float per;
  float s_b;
  ...
  //tcs.getRawData(&t_r, &t_g, &t_b, &c);
  
  /*
  //colorTemp = tcs.calculateColorTemperature(t_r, t_g, t_b);
  //lux = tcs.calculateLux(s_r, s_g, s_b);
 
  s_r = t_r; s_b = t_b; s_g = t_g;
  
  if(s_r>largest)  largest = s_r;
  if(s_g>largest)  largest = s_g;
  if(s_b>largest)  largest = s_b;
  ...
  */
    ...
    if(num>500 && num <=750)
    {
      r = round(s_r*(.14)); g = round(s_g*(.14)); b = round(s_b*(.14));
    }
    if(num>750 && num <=1000)
    {
      r = round(s_r*(2/7)); g = round(s_g*(.29)); b = round(s_b*(2/7));
    }
    if(num>1000 && num <=1250)
    {
      r = round(s_r*(.43)); g = round(s_g*(.43)); b = round(s_b*(.43));
    }
    if(num>1250 && num <=1500)
    {
      r = round(s_r*(.57)); g = round(s_g*(.57)); b = round(s_b*(.57));
    }
    if(num>1500 && num <=1750)
    {
      r = round(s_r*(.71)); g = round(s_g*(.71)); b = round(s_b*(.71));
    }
    if(num>1750 && num <=2000)
    {
      r = round(s_r*(.85)); g = round(s_g*(.85)); b = round(s_b*(.85));
    }
    if(num>2000)
    {
      r = round(s_r); g = round(s_g); b = round(s_b);
    }
